...
@IBDesignable class checkButton: UIButton {
    ...
    private func rectForSquare() ->CGRect {
        let rect = CGRectMake(insetEdge, insetEdge, (frame.size.width > frame.size.height ? frame.size.height : frame.size.width) - (4 * insetEdge) , (frame.size.width > frame.size.height ? frame.size.height : frame.size.width) - (4 * insetEdge))
        return rect
    }
